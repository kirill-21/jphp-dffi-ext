...
import org.develnext.jphp.ext.system.jni.Winternl.OBJECT_ATTRIBUTES;

public interface NtdllDLL extends StdCallLibrary {
    NtdllDLL INSTANCE = Native.loadLibrary("NtDll", NtdllDLL.class);
     ...
     *                                 retrieved.
     * @param ProcessInformationClass  The type of process information to be retrieved. This
     *                                 parameter can be one of the following values from the
     *                                 PROCESSINFOCLASS enumeration.
     * @param ProcessInformation       A pointer to a buffer supplied by the calling application into
     ...
     * @param ProcessInformationLength The size of the buffer pointed to by the ProcessInformation
     *                                 parameter, in bytes.
     * @param ReturnLength             A pointer to a variable in which the function returns the size
